...
public class HeapSort implements SortAlgorithm {
  ...
  /**
   * Transforms an integer array such that it represents a heap data structure, and iteratively
   * calls heapify on the heap in decrements so that the largest value is always at the end of
   * each newly unsorted portion. This results in an integer array that is sorted in
   * ascending order.
   *
  ...
  /**
   * Helper method that ensures that a heap follows a Max Heap structure, where the root/first
   * value is larger than all the other values that follow.
   *
   * @param arr An array representing/mimicking a heap.
   * @param heapSize The size of the heap, in this case the length of the array.
   * @param idx The index of the value to heapify (replace with the largest value, if found),
   *            usually the first index.
   */

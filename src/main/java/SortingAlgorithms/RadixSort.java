...
public class RadixSort implements SortAlgorithm {
   ...
   * its applicable negative and positive components, the components are then passed as arguments for the
   * radixSortAlgorithm method. Once sorted they are then merged into the original array such that the entire array is
   * then sorted.
   *
  ...
  public void sort(int[] arr) {
    ...
    if (negAndPosArrays[0] != null) { // sorts negative values, if present
      ...
      for (int i = 0; i < negArrLength; i++) {
        negAndPosArrays[0][i] *= -1; // (for sorting) temporarily transforms values from negative to positive
      }
      radixSortAlgorithm(negAndPosArrays[0]); // uses radix sort

      // transforms sorted values back to negative and reverses their order to maintain ascending pattern
